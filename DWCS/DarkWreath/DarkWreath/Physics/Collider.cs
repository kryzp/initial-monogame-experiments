...
namespace DarkWreath
{
	...
	public struct Hit
	{
		...
		{
			if (centered)
				return Vector2.DistanceSquared(me.WorldBounds.Center, Other.WorldBounds.Center);
			else
				return Vector2.DistanceSquared(me.WorldBounds.TopLeft, Other.WorldBounds.TopLeft);
		}
	}

	public class Collider
	{
		private int actorTransformId;
        ...
        public Collider()
        {
			...
			Polygon = new Polygon(Array.Empty<Vector2>());
			WorldPolygon = new Polygon(Array.Empty<Vector2>());
		...
		public void MakePolygon(Vector2[] vertices)
		{
		...
		public void MakePolygon(List<Vector2> vertices)
		{
			...
			this.WorldPolygon = new Polygon(vertices.ToArray());
		...
		{
			MakePolygon(new Polygon(new Vector2[]
			{
				new Vector2(0, 0),
				new Vector2(w, 0),
				new Vector2(w, h),
		...
		public Matrix GetFullTransformMatrix(bool centered = false)
		{
			...
			if (Actor != null) result *= Actor.Transform.Matrix;
			return result;
		}
		...
		{
			if (actorTransformId != 0)
				return;

			if (Actor != null)
			{
				if (Actor.TransformStamp == actorTransformId)
					return;
			}
			...
			// Update Axis and Points
			{
				...
				for (int i = 0; i < vertexCount; i++)
				{
					...
					axis[i] = (nextVertex - currentVertex).Normalized();
